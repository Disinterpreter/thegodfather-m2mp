class CMySQL { 
	...
	function RegisterPlayer(playerid,sPass)	{
		if(this.query("Insert into `" + this.sBaseName + "` (Name,Password) VALUES ('"+ getPlayerName ( playerid ) +"','"+ md5(sPass) +"')")!=0){log("CMySQL::RegisterPlayer Failed. Errno: "+this.iErr),return -1;}
	}
	function AccountExists(playerid) {
		if(this.query("Select `ID` from `" + this.sBaseName + "` where `Name` = '"+ getPlayerName(playerid) +"'")!=0){log("CMySQL::AccountExists Failed. Errno: "+this.iErr),return -1;}
		this.store();
		this.free();
		return !!this.iN;
	}
	function CheckLogin(playerid,sPass)	{
		if(this.query("Select * from `" + this.sBaseName + "` where `Name` = '"+ getPlayerName(playerid) +"' AND `Password` = '"+ md5(sPass) +"' LIMIT 1")!=0){log("CMySQL::CheckLogin Failed. Errno: "+this.iErr),return -1;}
		this.store();
